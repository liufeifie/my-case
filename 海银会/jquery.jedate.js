...
;(function (root, factory) {
  ...
  jedfn.initdates = function () {
    ...
    var formatDate = function (cls, boxcell) {
      ...
      $(that.opts.cellId || cls).append(dateDiv);
  ...
  jedfn.eachDays = function (ys, ms, ds, opts, boxCell) {
    ...
    var eachDays = function (yd, md) {
      ...
      for (var p = prevDaysNum - firstWeek + 1; p <= prevDaysNum; p++, count++) {
        ...
        daysArr.push({
          style: cls,
          ymd: prevM.y + '-' + prevM.m + '-' + p,
          day: p,
          d: (isfestival(prevM.y, prevM.m, p) + pmark),
          idx: didx++
        });
      }
      //本月的天数
      ...
      for (var n = 1, nlen = 42 - count; n <= nlen; n++) {
        ...
        daysArr.push({
          style: cls,
          ymd: nextM.y + '-' + nextM.m + '-' + n,
          day: n,
          d: (isfestival(nextM.y, nextM.m, n) + nmark),
          idx: didx++
        });
      }
      //将星期与日期拼接起来
  ...
  jedfn.eachHms = function (opts, boxCell) {
    ...
    $.each(new Array(ranges ? 1 : 2), function (m) {
      var timeList = $("<div/>", {"class": "timelist"}).css({
            width: ranges ? "100%" : "50%",
            float: ranges ? "" : "left"
          }),
  ...
  jedfn.clickDays = function (opts, boxCell) {
    ...
    if (opts.festival && lang.name == "cn") {
      ...
      tdCls.on("mouseover", function () {
        ...
        tipDiv.css({
          "z-index": (opts.zIndex == undefined ? 10000 + 5 : opts.zIndex + 5),
          top: tipPos.top,
          left: tipPos.left,
          position: "absolute",
          display: "block"
  ...
  jedfn.dateOrien = function (boxCls, valCls, pos) {
    ...
    if (that.fixed) {
      ...
      //根据目标元素计算弹层位置
      ortop = Math.max(tops + (pos ? 0 : jet.docScroll()) + 1, 1) + $(document).scrollTop() + "px", orleri = $(document).scrollLeft() + leris + "px";
    } else {
      //弹层位置位于页面上下左右居中
      ortop = "50%", orleri = "50%";
